from datasetClasses import MusdbDataset, LibriSpeechDataset, AccompanimentVocalData, SpeechData
...
class VocalCycleGAN:
	...
	def fit(self, musdb_dataset, librispeech_dataset):
		...
		self.discriminator_vocal = TsaiMiniRocketDiscriminator().to(device)
		...
		# create optimizers
		self.optimizer_GV  = optim.Adam(generator_vocal.parameters(),  lr=train_parameters["lr_G"],  betas=train_parameters["betas"])
		self.optimizer_GS = optim.Adam(generator_speech.parameters(), lr=train_parameters["lr_G2"], betas=train_parameters["betas"])
		self.optimizer_DV  = optim.Adam(discriminator_vocal.parameters(), lr=train_parameters["lr_D"], betas=train_parameters["betas"])
		self.optimizer_DS  = optim.Adam(discriminator_speech.parameters(), lr=train_parameters["lr_D"], betas=train_parameters["betas"])
	...
	def train_epoch(self):
    	...
    	for (acc_voc, speech) in tqdm(
        	zip(self.acc_voc_loader, self.speech_loader),
	        ...
	        # Generate reconstructed speech/vocal
     		fake_vocal_pad = self._transform_for_gen_2(fake_vocal)  # you must define this
        	...
        	fake_speech_pad = self._transform_for_gen_2(fake_speech)  # you must define this
    ...
    def _transform_for_gen_2(batch):
